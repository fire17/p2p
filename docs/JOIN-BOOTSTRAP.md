...
The generated Pages routes install Bun and p2p v0.3.8, connect an encrypted chat
...
```sh
curl -fsSL https://p2p.akeyo.io/join/<LISTENER_KEY> | sh
```

```powershell
irm https://p2p.akeyo.io/join/<LISTENER_KEY>.ps1 | iex
```
...
Both scripts download their platform installer from the exact v0.3.8 source
...
bootstrap forces the v0.3.8 source URL and Bun runtime; it does not downgrade a
